...
export const approvalRouter = router({
		...
		.mutation(async ({ ctx, input }) => {
			try {
				...
				if (error instanceof Error) {
					if (error.message === "Some images not found or unauthorized") {
						throw new TRPCError({
							code: "BAD_REQUEST",
	 ...
	 */
	reject: publicProcedure.input(rejectImageSchema).mutation(async ({ input }) => {
		try {
			const result = await approvalService.reject(input.token, input.imageId, {
				feedback: input.feedback,
			});
