...
import { S3Provider, type S3Config } from "./providers/S3Provider";
import { R2Provider, type R2Config } from "./providers/R2Provider";
import {
	GoogleDriveProvider,
	type GoogleDriveConfig,
} from "./providers/GoogleDriveProvider";
import { NASProvider, type NASConfig } from "./providers/NASProvider";
...
export class StorageFactory {
	 ...
	 */
	static async createDefaultForUser(
		userId: string,
	): Promise<IStorageProvider> {
		...
		const defaultConfig =
			config.find((c) => c.isDefault) || config[0];
